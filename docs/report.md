# Thesis Defense Studio：基于大语言模型的 AI 模拟答辩 Web 平台设计与实现

## 摘要

针对高中生课题答辩、本科课程论文答辩普遍存在模拟演练机会稀缺、论文自查效率低下、缺少客观专业反馈三大现实困境，本研究以“市场调研 → Web构建 → 评估 → 迭代”为主线，设计并实现一套基于大语言模型（LLM）的 AI 模拟答辩 Web 平台 **Thesis Defense Studio**。

首先通过用户问卷调研识别答辩备考阶段用户真实痛点；其次搭建系统整体架构，依托大模型实现论文全文智能审阅、AI 智能评委随机出题、模拟答辩问答、多维度量化评分与改进建议输出等核心功能；同时针对 AI 幻觉、长文本溢出、用户论文隐私泄露三大风险提出对应技术方案。

本平台构建了“需求发起 → AI生成 → 实操演练 → 用户反馈 → 模型迭代”的完整服务闭环。

研究结果表明，Thesis Defense Studio 能够为学生提供低成本、可重复使用的全真答辩模拟环境，有效弥补传统答辩备考资源不足的问题，为 AI 赋能学术答辩备考系统开发提供实践参考。
...
随着新课改项目的推进，论文答辩与演讲成为评价高中生学术素养的重要环节。

然而完成论文写作并不意味着能够清楚讲述研究内容。许多学生主要依靠反复阅读论文、背诵讲稿或制作演示文稿进行准备，缺少从评审视角审视研究的机会。
...
本科课程论文答辩同样存在类似需求，但不同学段研究基础和评价标准不同，支持方式也需要针对性设计。

大语言模型（LLM）能够通过自然语言交互和虚拟问答补足这一空缺。
...
大语言模型建立在 Transformer 架构基础上。

Transformer 通过 Self-Attention（自注意力机制）动态计算各 Token 之间的关联权重，从而捕捉长距离语义依赖关系。
...
例如 GPT-3 使用约 4100 亿 Token 进行训练。

### 对齐训练

预训练结束后进入 Alignment 阶段：
...
## 2.3 教育场景下 AI 答辩系统理论基础

UNESCO《Guidance for Generative AI in Education and Research》指出：
...
Meyer 等（2024）开展随机对照实验：

- 样本量：459 名高中生
- GPT-3.5-turbo 提供论文反馈
- 干预时长：90分钟
